    ...
    <div>
        ...
        <el-row class="main-body">
            <el-table @sort-change='sortFunc' header-row-class-name="thead" :data="tableData" stripe v-loading="loading" element-loading-text="加载中...">
                <el-table-column prop="surveyTitle" label="调查名称" min-width="200"></el-table-column>
...
export default class Companyreg extends Vue {
  ...
  // 恢复删除
  onResume(e: any): void {
    this.$confirm("您确定要还原这篇调查吗?", "提示", {
      confirmButtonText: "确定",
      cancelButtonText: "取消"
    })
      .then(() => {
        Survey.recover_survey_admin({
          data: { surveyId: e.surveyId }
        }).then(res => {
          if (res.code === 0) {
            this.$notify({
              title: "成功",
  ...
  onRadicalDel(e: any): void {
    this.$confirm("你确定要彻底删除这篇调查吗?", "提示", {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning"
    })
      .then(() => {
        Survey.delete_survey_admin({ data: { surveyId: e.surveyId } }).then(
          res => {
            if (res.code === 0) {
              this.$notify({
                title: "成功",
  ...
  getList(type: any) {
    ...
    this.loading = true;
    Survey.recycle_survey_list_admin({ data })
      .then(res => {
        if (res.code === 0) {
