  ...
  <div>
    <el-row class="main-top">
      <el-form :inline="true" class="btn-main">
        <el-form-item label="">
          <el-button type="primary" icon="plus" @click="createdBtn" v-if="prm.isCreate">创建链接</el-button>
        </el-form-item>
      </el-form>
      <el-form :inline="true">
        <el-form-item label="">
          <el-input placeholder="链接标题" v-model="params.query" class='w230' @keyup.13.native="searchKey">
            <el-button class="clickBtn" @click="searchKey" slot="append" icon="el-icon-search"></el-button>
          </el-input>
        </el-form-item>
        <el-form-item>
          <el-select placeholder="链接状态" class='w150' v-model="params.state" @change="getList" clearable @clear="params.state = ''">
    ...
    <el-row class="main-body">
      <el-table ref="mainTable" @selection-change="handleSelectionChange" @sort-change='sortFunc' header-row-class-name="thead" :data="tableData" stripe v-loading="loading" element-loading-text="加载中...">
        ...
        <el-table-column prop="img" label="链接封面" width='100' align='center'>
          <template slot-scope="scope">
            <img class='img-bor' :src="scope.row.img ? ($store.state.configData.imgDomain + scope.row.img+$imgSize['30x30']) : ($store.state.configData.imgDomain + '/default/cdwork/course.jpg' + $imgSize['30x30'])"
            />
          </template>
        </el-table-column>
        <el-table-column prop="title" label="链接标题" min-width="210">
          <!-- <template slot-scope='scope'>
            <a style="cursor: pointer" @click="onClickTitle(scope.row)">{{scope.row.title}}</a>
          </template> -->
        </el-table-column>
        <el-table-column prop="categoryTitle" label="链接分类" min-width='100'></el-table-column>
        <el-table-column prop="linkUrl" label="链接地址" min-width='150'></el-table-column>
    ...
    <!-- 预览弹出框 -->
    <!-- <el-dialog title="链接预览" width="400px" style="height: 900px; overflow: hidden;" class="artic-dlog" :visible.sync="isShowPreview">
...
  }
  on_off(e: any): void {
    ...
    Link.online_offline_link_admin({ data }).then(res => {
      if (res.code === 0) {
        this.$notify({
          title: "提示",
  ...
  delCheck(){
    ...
    let id = JSON.stringify(this.selectData.map(i => i.linkId));
    this.delId = id.replace(/"|\[|\]/g,'');
    this.del();
  }
  del(e: any): void {
    this.$confirm('确认删除所选链接?', '删除', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
    }).then(() => {
      let data = {
        linkIds: e ? e.linkId : this.delId
      };
      Link.delete_link_admin({ data }).then(res => {
        if (res.code === 0) {
          this.$notify({
            title: "提示",
            message: "删除成功！",
  ...
  // onClickTitle(e: any){
  //   // 生产环境
  //   this.previewSrc = "https://m.weijiuye.com.cn/pages/discover/publiclink/detail?linkId=" + e.linkId;
