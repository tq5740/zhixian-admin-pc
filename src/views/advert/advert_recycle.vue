    ...
    <div>
        <el-row class="main-top">
            <el-form :inline="true">
                <el-form-item label="">
                    ...
                    <el-input placeholder="广告名称" v-model="params.query" class='w230' @keyup.13.native="searchKey">
                        <el-button class="clickBtn" @click="searchKey" slot="append" icon="el-icon-search"></el-button>
                    </el-input>
                </el-form-item>
                <el-form-item>
                    <el-cascader placeholder="广告分类" filterable :options="advertClassify" :props="keyName" v-model="categoryId" @change="ClassifyId" change-on-select clearable></el-cascader>
                </el-form-item>
                <!-- <el-form-item>
        ...
        <el-row class="main-body">
            <el-table @sort-change='sortFunc' header-row-class-name="thead" :data="tableData" stripe v-loading="loading" element-loading-text="加载中...">
                <el-table-column prop="advertTitle" label="广告名称" min-width="200"></el-table-column>
                <el-table-column prop="categoryTitle" label="广告分类" min-width='140'></el-table-column>
...
export default class Companyreg extends Vue {
  ...
  // 恢复删除
  onResume(e: any): void {
    this.$confirm("您确定要还原这篇广告吗?", "提示", {
      confirmButtonText: "确定",
      cancelButtonText: "取消"
    })
      .then(() => {
        Advert.rec_advert_admin({
  ...
  onRadicalDel(e: any): void {
    this.$confirm("你确定要彻底删除这篇广告吗?", "提示", {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning"
    })
      .then(() => {
        Advert.delete_advert_admin({ data: { advertId: e.advertId } }).then(
          res => {
            if (res.code === 0) {
              this.$notify({
                title: "成功",
  ...
  getList(type: any) {
    ...
    this.loading = true;
    Advert.recycle_advert_list_admin({ data })
      .then(res => {
        if (res.code === 0) {
