...
<template>
  <div class="category">
    ...
    <el-row class="main-top">
      <el-form :inline="true">
        <el-form-item label="" class="btn-main">
          <el-button type="primary" @click.native="createJobClassify">创建广告分类</el-button>
        </el-form-item>
        <el-form-item>
    ...
    <el-row class="main-body" v-loading="loading" element-loading-text="加载中">
      <el-tree :filter-node-method="filterNode" ref="classify" :data="advertClassify" :props="defaultProps" node-key="value" :default-expand-all="false" :expand-on-click-node="true" :empty-text="`暂无更多数据`">
    ...
    <el-dialog :close-on-click-modal="false" :title="title" v-if="showBox" :visible.sync="showBox">
      <el-form :label-width="`100px`">
        ...
        <el-form-item label="上级广告分类">
          <el-cascader v-model="moverClassify" class='w80per' placeholder="请选择" :options="advertClassifyClone" :props="defaultProps" @change='change' filterable change-on-select></el-cascader>
        </el-form-item>
...
@Component({})
export default class Advertclassify extends Vue {
  advertClassify: any = [];
  advertClassifyClone: any = [];
  ...
  getCategory() {
    ...
    Category.category_list_advert({ data }).then(res => {
      if (res.code === 0) {
        ...
        this.advertClassifyClone = [this.top, ...res.data];
      }
    });
  }
  async getOrgList() {
  ...
  createSaveBtn() {
    ...
    if (this.title === "创建广告分类") {
      Category.create_category_advert({ data })
        .then(res => {
          if (res.code === 0) {
            this.$notify({
              title: "提示",
      ...
      Category.update_category_advert({ data })
        .then(res => {
          if (res.code === 0) {
            this.$notify({
              title: "提示",
  ...
  remove(node: any, datas: any) {
    ...
    this.$confirm("确认删除该分类吗?", "提示", {
      ...
      .then(() => {
        ...
        this.isPending = true;
        Category.delete_category_advert({ data })
          .then(res => {
            if (res.code === 0) {
              this.$notify({
                title: "提示",
