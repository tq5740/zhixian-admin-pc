    ...
    <div>
        ...
        <el-row class="main-body">
            <el-table @sort-change='sortFunc' @row-click="rwoClick" header-row-class-name="thead" :data="tableData" stripe v-loading="loading" element-loading-text="加载中...">
                ...
                <el-table-column prop="major" label="职业工种" min-width='140'></el-table-column>
                <el-table-column prop="level" label="证书级别" min-width='100'></el-table-column>
                <el-table-column prop="issuDateStr" label="发证日期" width='110'></el-table-column>
                <el-table-column prop="stateStr" label="状态" width='80'></el-table-column>
                <el-table-column prop="createUserName" label="申请人" min-width='100'></el-table-column>
...
import SubsidyApply from "@api/subsidyApply";
...
export default class Complaintlist extends Vue {
  ...
  saveBtn() {
    ...
    this.isPending = true;
    SubsidyApply.auditCertificateAdmin({ data })
      .then(res => {
        if (res.code === 0) {
          this.$notify({
            title: "提示",
  ...
  del(e: any) {
    this.$confirm(`是否将 "${e.realName}" 删除? `, "提示", {
      ...
      .then(() => {
        ...
        SubsidyApply.deleteCertificateAdmin({ data })
          .then(res => {
            if (res.code === 0) {
              this.$notify({
                title: "提示",
  ...
  audit(e: any) {
    this.auditObj.certificateId = e.certificateId;
    this.showAudit = true;
  }
  sortFunc() {
  ...
  getList(type: any) {
    ...
    SubsidyApply.certificateListAdmin({ data })
      .then(res => {
        if (res.code === 0) {
          res.data.result.forEach((i: any) => {
            i.createTimeStr = this.$formatDate(i.createTime);
            i.issuDateStr = this.$formatDate(i.issuDate, "YYYY-MM-DD");
