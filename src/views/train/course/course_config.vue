<template>
  <div v-loading="loading">
    <el-form class="main-top">
      <el-form-item label="课程是否需要审核 : " label-width="140px">
...
export default class Photocfg extends Vue {
  ...
  getConfig() {
    this.loading = true;
    let data = {};
    Course.query_course_config_admin({ data })
    .then(res => {
        if (res.code === 0) {
            res.data.isAuditCourse === true ? this.tof = 1 : this.tof = 2;
  ...
  save() {
    this.isPending = true;
    let data = {
      isAuditCourse: this.tof === 1 ? true : false
    };
    Course.update_course_config_admin({ data })
      .then(res => {
        if (res.code === 0) {
          this.$notify({
            title: "提示",
