  ...
  <div>
    <el-row class="main-top">
      <el-form :inline="true" class="btn-main">
        <el-form-item label="">
          <el-button type="primary" icon="plus" @click="created_course" v-if="prm.isCreate">创建课程</el-button>
        </el-form-item>
      </el-form>
      <el-form :inline="true">
        <el-form-item label="">
          <el-input style="display:none"></el-input>
          <el-input placeholder="课程标题" v-model="params.query" class='w230' @keyup.13.native="searchKey" clearable>
            <el-button @click="searchKey" slot="append" icon="el-icon-search"></el-button>
          </el-input>
        </el-form-item>
        <el-form-item>
          <el-cascader filterable :options="courseClassify" :props="keyName" v-model="categoryId" @change="ClassifyId" change-on-select clearable placeholder="课程分类"></el-cascader>
        </el-form-item>
        <el-form-item>
          <el-select placeholder="课程状态" class='w150' v-model="params.state" @change="getList" clearable @clear="params.state = ''">
    ...
    <el-row class="main-body">
      <el-table ref="mainTable" @selection-change="handleSelectionChange" @sort-change='sortFunc'
        ...
        <el-table-column prop="categoryTitle" label="课程分类" min-width='100'></el-table-column>
    ...
    <el-dialog width="70%" v-if="createEditOne" :title="title" :visible.sync="createEditOne" :close-on-click-modal="false" :before-close="closeOne">
      <create-edit-one @createEditOneOver="createEditOneOver" :queryData="queryCourseData" :title="{'title':title}"></create-edit-one>
    </el-dialog>
    <el-dialog width="70%" v-if="createEditTwo" :title="title" :visible.sync="createEditTwo" :close-on-click-modal="false" :before-close="closeTwo">
      <create-edit-two @createEditTwoOver='createEditTwoOver' @createEditTwoOverUp='createEditTwoOverUp' @createEditTwoOverDown='createEditTwoOverDown' :queryData="queryCourseData" :title="{'title':title}" :isShow='isShow'></create-edit-two>
    </el-dialog>
    <el-dialog width="70%" v-if="createEditThree" :title="title" :visible.sync="createEditThree" :close-on-click-modal="false" :before-close="closeThree">
      <create-edit-three :options='courseData' @createEditThreeOverUp="createEditThreeOverUp" @createEditThreeOver="createEditThreeOver" :isShow='isShow'></create-edit-three>
    </el-dialog>
    <el-dialog width="70%" v-if="statisticsUser" title="课程用户报表" :visible.sync="statisticsUser" :close-on-click-modal="false" :before-close="closeUser">
      <statistics-user @statisticsUserOver="statisticsUserOver" from="course"></statistics-user>
    </el-dialog>
    <el-dialog width="50%" v-if="statisticsState" title="统计" :visible.sync="statisticsState" :close-on-click-modal="false">
      <el-table header-row-class-name="thead" :data="statisticsData" border v-loading="loading" header-align="center" element-loading-text="加载中...">
        <el-table-column prop="chapterNumber" label="章节数" align="center" min-width="80"></el-table-column>
        ...
        <el-table-column label="学习人数" align="center" min-width="80">
          <template slot-scope='scope'>
            <a style="cursor: pointer" @click="studyNumberFun(scope.row)">{{scope.row.studyNumber}}</a>
          </template>
        </el-table-column>
...
  }
  courseOnlineOffline() {
    ...
    Course.online_offline_course({ data })
      .then(res => {
        if (res.code === 0) {
  ...
  recommend(e: any): void {
    ...
    Course.is_recommend_course({ data }).then(res => {
      if (res.code === 0) {
        this.getList();
      }
    });
  ...
  on_off(e: any): void {
    ...
    Course.online_offline_course({ data }).then(res => {
      if (res.code === 0) {
        this.$notify({
          title: "提示",
  ...
  saveBtn() {
    ...
    this.isPending = true;
    Course.audit_course_admin({ data })
      .then(res => {
        if (res.code === 0) {
          this.$notify({
            title: "提示",
  ...
  del(): void {
    ...
    let data = {
      courseIds: this.delCourseId,
      reason: this.delCause
    };
    Course.del_course_admin({ data }).then(res => {
      if (res.code === 0) {
        this.$notify({
          title: "提示",
          message: "删除成功！",
  ...
  statistics(e: any) {
    if(e){
      ...
      arr.id = e.courseId;
      arr.title = e.courseTitle;
      this.$store.commit('setCourseData', arr)
    }
    let data = {
      courseId: this.$store.state.courseData.id
    };
    Course.query_course_statistics_admin({ data }).then(res => {
      if (res.code === 0) {
        let arr = [];
        arr.push(res.data);
  ...
  // 创建课程
  created_course(): void {
    this.$store.state.courseData.id = '';
    this.createEditOne = true;
    this.title = "创建课程";
  }
  ...
  addClass(e: any) {
    this.$store.commit('setCourseData', { id: e.courseId });
