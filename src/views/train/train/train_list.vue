  ...
  <div>
    <el-row class="main-top">
      <el-form :inline="true" class="btn-main">
        <el-form-item label="">
          <el-button type="primary" icon="plus" @click="created_train" v-if="prm.isCreate">创建培训</el-button>
        </el-form-item>
      </el-form>
      <el-form :inline="true">
        <el-form-item label="">
        ...
        <el-form-item>
          <el-cascader filterable :options="trainClassify" :props="keyName" v-model="categoryId" @change="ClassifyId" change-on-select clearable placeholder="培训分类"></el-cascader>
        </el-form-item>
        <el-form-item>
          <el-select placeholder="培训状态" class='w150' v-model="params.state" @change="getList" clearable @clear="params.state = ''">
            <el-option v-for="(i) in stateArr" :key="i.text" :label="i.text" :value="i.id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
          <el-select placeholder="培训类型" class='w150' v-model="params.trainType" @change="getList" clearable @clear="params.trainType = ''">
            <el-option v-for="(i) in typeArr" :key="i.text" :label="i.text" :value="i.id"></el-option>
          </el-select>
        </el-form-item>

        <el-form-item>
          <el-date-picker v-model="time" start-placeholder="开始日期" end-placeholder="结束日期" type="daterange" placeholder="选择时间" @change='handleChangeTime' value-format='yyyy-MM-dd'
          > </el-date-picker>
        </el-form-item>
      </el-form>
    </el-row>
    <el-row class="main-body">
      <el-table ref="mainTable" @selection-change="handleSelectionChange" @sort-change='sortFunc'
        ...
        <el-table-column prop="trainImg" label="培训封面" width='100' align='center'>
          <template slot-scope="scope">
            <img class='img-bor' :src="scope.row.trainImg ? ($store.state.configData.imgDomain + scope.row.trainImg+$imgSize['30x30']) : ($store.state.configData.imgDomain + '/default/cdwork/train.jpg' + $imgSize['30x30'])"
            />
          </template>
        </el-table-column>
        <el-table-column prop="trainTitle" label="培训名称" min-width="140"></el-table-column>
        <el-table-column prop="typeStr" label="培训类型" min-width='100'></el-table-column>
        <el-table-column prop="categoryTitle" label="培训分类" min-width='100'></el-table-column>
        ...
        <el-table-column prop="" label="操作" width="490">
          <template slot-scope='scope'>
            ...
            <el-button type="text" @click.native="addClass(scope.row)">课程</el-button>
            <el-button type="text" @click.native="choiceUser(scope.row)">报名</el-button>
    ...
    <el-dialog width="70%" v-if="createEditOne" :title="title" :visible.sync="createEditOne" :close-on-click-modal="false" :before-close="closeOne">
      <create-edit-one @createEditOneOver="createEditOneOver" :queryData="queryTrainData" :title="{'title':title}"></create-edit-one>
    </el-dialog>
    <el-dialog width="70%" v-if="createEditTwo" :title="title" :visible.sync="createEditTwo" :close-on-click-modal="false" :before-close="closeTwo">
      <create-edit-two @createEditTwoOver='createEditTwoOver' @createEditTwoOverUp='createEditTwoOverUp' @createEditTwoOverDown='createEditTwoOverDown' :queryData="queryTrainData" :title="{'title':title}" :isShow='isShow'></create-edit-two>
    </el-dialog>
    <el-dialog width="70%" v-if="createEditThree" :title="title" :visible.sync="createEditThree" :close-on-click-modal="false" :before-close="closeThree">
      <create-edit-three :options='trainData' @createEditThreeOverUp="createEditThreeOverUp" @createEditThreeOver="createEditThreeOver" :isShow='isShow'></create-edit-three>
    </el-dialog>
    <el-dialog width="70%" v-if="statisticsUser" title="培训用户报表" :visible.sync="statisticsUser" :close-on-click-modal="false" :before-close="statisticsUserOver">
      <statistics-user @statisticsUserOver="statisticsUserOver" @courseReport="courseReport" from="train"></statistics-user>
    </el-dialog>
    <el-dialog width="50%" v-if="statisticsState" title="统计" :visible.sync="statisticsState" :close-on-click-modal="false">
      <el-table header-row-class-name="thead" :data="statisticsData" border v-loading="loading" header-align="center" element-loading-text="加载中...">
        <el-table-column prop="resourceNumber" label="资源数量" align="center" min-width="80"></el-table-column>
        <el-table-column prop="browseNumber" label="浏览量" align="center" min-width="80"></el-table-column>
        <el-table-column prop="complimentNumber" label="点赞数" align="center" min-width="80"></el-table-column>
        <el-table-column prop="commentNumber" label="评论数" align="center" min-width="80"></el-table-column>
        <el-table-column label="培训人数" align="center" min-width="80">
          <template slot-scope='scope'>
            <a style="cursor: pointer" @click="studyNumberFun(scope.row)">{{scope.row.trainNumber}}</a>
          </template>
        </el-table-column>
        <el-table-column label="平均进度" align="center" min-width="80">
          <template slot-scope="scope">
            {{scope.row.progress && Math.round(scope.row.progress * 100) + "%" || "-"}}
          </template>
        </el-table-column>
        <el-table-column prop="length" label="学习总时长" align="center" min-width="80">
          <template slot-scope="scope">
            {{scope.row.length ? $formatSeconds(scope.row.length) : "-"}}
          </template>
        </el-table-column>
        <el-table-column prop="score" label="平均成绩" align="center" min-width="80">
          <template slot-scope="scope">
...
  }
  trainOnlineOffline() {
    ...
    Train.online_offline_train({ data })
      .then(res => {
        if (res.code === 0) {
  ...
  delCheck(e: any){
    ...
    let id = e && e.trainId ? JSON.stringify(e.trainId) : JSON.stringify(this.selectData.map(i => i.trainId));
    this.delTrainId = id.replace(/"|\[|\]/g,'');
    this.isDelShow = true;
  }
  ClassifyId(e: any) {
    if (e.length) {
  ...
  recommend(e: any): void {
    ...
    Train.is_recommend_train({ data }).then(res => {
      if (res.code === 0) {
        this.getList();
      }
    });
  ...
  on_off(e: any): void {
    ...
    Train.online_offline_train({ data }).then(res => {
      if (res.code === 0) {
        this.$notify({
          title: "提示",
  ...
  del(): void {
    ...
    let data = {
      trainIds: this.delTrainId,
      reason: this.delCause
    };
    Train.del_train_admin({ data }).then(res => {
      if (res.code === 0) {
        this.$notify({
          title: "提示",
          message: "删除成功！",
  ...
  statistics(e: any) {
    if(e){
      ...
      arr.id = e.trainId;
      arr.title = e.trainTitle;
      this.$store.commit('setTrainData', arr)
    }
    let data = {
      trainId: this.$store.state.trainData.id
    };
    Train.query_train_statistics_admin({ data }).then(res => {
      if (res.code === 0) {
        let arr = [];
        arr.push(res.data);
  ...
  edit(e: any): void {
    ...
    Train.query_train_admin({ data })
      .then(res => {
        if (res.code === 0) {
          this.queryTrainData = res.data;
      ...
      .then(res => {
        if (res.code === 0) {
          ...
          res.data.result.forEach((i: any) => {
            ...
            i.typeStr = this.$mapTypeStr(i.trainType, this.typeArr);
          });
  ...
  addClass(e: any) {
    this.$store.commit('setTrainData', { id: e.trainId });
