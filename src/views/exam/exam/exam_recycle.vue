    ...
    <div>
        <el-row class="main-top">
            <el-form :inline="true">
                <el-form-item label="">
                    ...
                    <el-input placeholder="考试名称" v-model="params.query" class='w230' @keyup.13.native="searchKey">
                        <el-button class="clickBtn" @click="searchKey" slot="append" icon="el-icon-search"></el-button>
                    </el-input>
                </el-form-item>
                <el-form-item>
                    <el-cascader placeholder="考试分类" filterable :options="examClassify" :props="keyName" v-model="categoryId" @change="ClassifyId" change-on-select clearable></el-cascader>
                </el-form-item>
            </el-form>
        </el-row>
        <el-row class="main-body">
            <el-table @sort-change='sortFunc' header-row-class-name="thead" :data="tableData" stripe v-loading="loading" element-loading-text="加载中...">
                <el-table-column prop="examTitle" label="考试名称" min-width="200"></el-table-column>
                <el-table-column prop="categoryTitle" label="考试分类" min-width='140'></el-table-column>
...
export default class Companyreg extends Vue {
  ...
  // 恢复删除
  onResume(e: any): void {
    this.$confirm("您确定要还原这篇考试吗?", "提示", {
      confirmButtonText: "确定",
      cancelButtonText: "取消"
    })
      .then(() => {
        Exam.recover_exam_admin({
          data: { examId: e.examId }
        }).then(res => {
          if (res.code === 0) {
            this.$notify({
              title: "成功",
  ...
  onRadicalDel(e: any): void {
    this.$confirm("你确定要彻底删除这篇考试吗?", "提示", {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning"
    })
      .then(() => {
        Exam.delete_exam_admin({ data: { examId: e.examId } }).then(
          res => {
            if (res.code === 0) {
              this.$notify({
                title: "成功",
  ...
  getList(type: any) {
    if (type !== 'type') { 
      if (this.params.query || this.params.categoryId) {
        this.params.pageNum = 1;
      }
    };
    let data = { ...this.params };
    this.loading = true;
    Exam.recycle_exam_list_admin({ data })
      .then(res => {
        if (res.code === 0) {
